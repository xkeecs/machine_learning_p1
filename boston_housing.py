...
def explore_city_data(city_data):
    ...
    # Mean features
    print np.mean(housing_features, 0)
...
def performance_metric(label, prediction):
    ...
    #return smetrics.explained_variance_score(label, prediction)
    return smetrics.median_absolute_error(label, prediction)
...
def fit_predict_model(city_data):
    ...
    train_err = 1000*np.ones(len(depth))
    test_err = 1000*np.ones(len(depth))
    max_index = 0
    cur_error = 0
    ###################################
    ...
    for i, d in enumerate(depth):
        ...
        # Find the performance on the testing set
        cur_error = performance_metric(y_test, regressor.predict(X_test))
        if (i > 0 and min(test_err) > cur_error):
            max_index = i
